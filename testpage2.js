...
function teamDupes(week, fantasyID, numDupeTeamsAllowed, position, teamRoster, teamName, playerGametimeArray) {
	...
	$.ajax({
	    ...
	    success: function(response) {
			...
			if ((selectedPlayerTeam != positionToTeam[newPosition] && selectedPlayerTeam != undefined && (counts[selectedPlayerTeam] <= 1 && dupeTeams >= numDupeTeamsAllowed)) || newPlayerCount > 2/* || moreThanTwoDupeTeams*/) {  // If selected team has >= 1 use and we've hit the limit of dupe teams
