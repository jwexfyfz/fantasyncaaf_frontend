	    ...
	    success: function(response) {
		  ...
		  for(i = 0; i < phpResponse.length; i++) {
			  if(phpResponse[i]["teamID"] == teamID) {
				  ...
				  $('#standingsTable').append('<tr><td class="standingsTableRow">'+(i+1)+'</td><td class="standingsTableRow">'+getNameFromID[phpResponse[i]["teamID"]]+'</td><td class="standingsTableRow">'+phpResponse[i]["wins"]+'-'+phpResponse[i]["losses"]+'</td><td class="standingsTableRow">'+phpResponse[i]["divisionWins"]+'-'+phpResponse[i]["divisionLosses"]+'</td><td class="standingsTableRow">'+phpResponse[i]["pointsFor"]+'</td><td class="standingsTableRow currentTeam">'+phpResponse[i]["pointsAgainst"]+'</td></tr>');
			  }
		  }
